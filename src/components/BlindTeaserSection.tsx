...
function BlurredCard({ card, index, isVisible }: { card: typeof blurredCards[0]; index: number; isVisible: boolean }) {
    return (
        <div
    ...
    return (
        <section className="teaser section" id="teaser" ref={ref}>
            <div className="container">
                ...
                {/* Blurred cards — static blur with gradient overlay */}
                <div className="teaser__blurred-wrap">
                    <div className="teaser__blurred-grid">
                        {blurredCards.map((card, i) => (
                            <BlurredCard key={i} card={card} index={i} isVisible={isVisible} />
                        ))}
                    </div>
